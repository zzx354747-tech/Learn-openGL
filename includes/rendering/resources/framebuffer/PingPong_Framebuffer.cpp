...
PingPongFramebuffer::PingPongFramebuffer(int width, int height)
{
        initFramebuffer(width, height);
    }

void PingPongFramebuffer::resize(int width, int height)
{
        for (int i = 0; i < 2; ++i)
        {
...
GLuint PingPongFramebuffer::getTextureID(int index) const
{
        if (index < 0 || index >= 2)
        {
            std::cerr << "ERROR::PINGPONG_FRAMEBUFFER:: index out of range!" << std::endl;
...
void PingPongFramebuffer::initFramebuffer(int width, int height)
{
